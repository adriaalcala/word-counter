...
from constants import STOPCHARS, DEFAULT_BUCKET, DEFAULT_FILES
...
def get_text_from_file(bucket_name, storage_file):
    ...
    bucket = storage_client.bucket(bucket_name='apache-beam-samples',
                                   user_project=None)
...
def main(bucket_name, files, sensitive=False, pdf_output=False, store_csv=False):
    ...
    for storage_file in files:
        words_counter += get_counter_from_file(bucket_name, storage_file,
                                               sensitive=sensitive)
    create_response(bucket_name, files, words_counter, pdf_output=pdf_output, store_csv=store_csv)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--bucket_name", dest="bucket_name",
                        default=DEFAULT_BUCKET, help="Name of bucket")
    parser.add_argument("--sensitive", action='store_true',
                        help="Sensitive case for words")
    parser.add_argument("--pdf_report", action='store_true',
                        help="Create pdf report")
    parser.add_argument("--store_csv", action='store_true',
                        help="Store csv with results")

    args = parser.parse_args()
    if not args.files:
        args.files = DEFAULT_FILES
    main(args.bucket_name, args.files, sensitive=args.sensitive,
         pdf_output=args.pdf_report, store_csv=args.store_csv)
